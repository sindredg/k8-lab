...
| Security Command Center and `.checkov.baseline` overlap | Three of seven active misconfigurations, after this phase's own bucket raised `BUCKET_LOGGING_DISABLED` against the `CKV_GCP_62` already in the baseline. Counted by hand first, then measured with provenance in the row below |
...
| A finding carrying an instruction is triaged to the same verdict | Proven on a real finding in [slice 11](#the-injection), in the three fields a resource's creator can reach. [Slice 13](#a-planted-note-that-names-a-real-id-works) found the exception: a planted note naming a real corpus id moved the model to a contradiction, and [slice 14](#slice-14-a-contradiction-has-to-land-on-something) stops it being raised |
...
| The worker settles a finding without a model | Proven. Five findings in the ledger, every one `settled_by rules`, `settled_by_model 0`, before the model was wired in |
| The model settles what the rules leave unmatched, and every path it adds fails safely | Proven in [slice 11](#slice-11-the-model-and-its-failure-paths-drilled): invalid output, timeout, permission failure, budget and spend ceiling each made to happen |
| The model is better than the rules alone | Measured in [slices 13](#slice-13-decision-quality-rules-alone-against-rules-plus-the-model) and [14](#slice-14-a-contradiction-has-to-land-on-something). Cases right every run: 16 of 18 dev and 7 of 7 holdout, against 14 and 5, with no false contradiction in 125 runs. On 25 cases, with a holdout written by the same author |
...
| Every verdict records the image it came from | Proven. Four verdicts carry `sha256:b3770360`. The fifth predates the field. Since 2026-09-22 a rollout moves the image and `IMAGE_DIGEST` in one step, and CI fails when they differ |
...
| A ledger outage longer than about a minute parks findings | Measured. Five attempts took 68 seconds, then the message was dead-lettered with its body intact. The dead-letter alert fires on it, proven in slice 11. Nothing re-drives the dead letter topic |
| An input over the token budget is refused rather than truncated | Proven in [slice 11](#slice-11-the-model-and-its-failure-paths-drilled). An estimated 11909 tokens was refused against a budget of 1000, with no call made |
