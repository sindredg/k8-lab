...
Result: no warning about existing Pods, so the running workload already satisfies `baseline`. Both Pods kept running with zero restarts.
...
Result: `warn` reported three `restricted` violations and applied the Deployment anyway.

- `allowPrivilegeEscalation != false`
- unrestricted capabilities
- `runAsNonRoot != true`

These are the same gaps kube-linter reports in CI.
...
Result: changing `serviceAccountName` altered the Pod template, so a new ReplicaSet replaced both Pods. Both run as `nginx`, and the API token directory does not exist inside the container.
...
Result: refused with `violates PodSecurity "baseline:v1.35": privileged`. Nothing was created, and the two NGINX Pods were unaffected.

| Mode | Message | Outcome |
| --- | --- | --- |
| `enforce` | `violates` | rejected |
| `warn` | `would violate` | allowed |
...
- Added a `LimitRange` giving containers default CPU and memory values matching the NGINX workload.
- Added a `ResourceQuota` limiting the namespace to 1 CPU and 1Gi requested, 2 CPU and 2Gi limited, and 10 Pods.
- Applied the LimitRange first, because the quota rejects Pods missing the values it tracks.

### Sizing

| Resource | Per `e2-standard-2` node | Cluster maximum, 3 nodes |
| --- | --- | --- |
| CPU allocatable | 1930m | 5790m |
| Memory allocatable | 6170272Ki, about 5.9Gi | about 17.6Gi |

GKE keeps the remainder for the operating system, Kubernetes services, and system Pods.

![CPU and memory available on the node](../images/quota-node-allocatable.png)

The quota is a policy limit, not a measure of cluster capacity. A Pod must pass the quota check and then find space on a node. These are separate checks.
...
Result: the quota counted the running workload correctly. Two Pods request 100m CPU and 128Mi memory in total, limited to 500m CPU and 256Mi memory.
...
Result: the Pod received the default values.
...
The LimitRange sets defaults only, with no per-container minimum or maximum, which is what made it possible to test the quota separately. With a maximum in place the LimitRange would have rejected the test Pod first. A shared namespace should normally use both controls.
...
- A `default-deny` NetworkPolicy selecting every Pod in the namespace, both directions.
- `allow-dns`, permitting egress to the cluster DNS Pods on port 53.
- `nginx-allow-http`, admitting traffic to the NGINX Pods from Pods labelled `nginx-client`.
- `nginx-client-allow-egress`, letting those clients open the connection.
- No egress allowance for the NGINX Pods, because the workload makes no outbound connections.

### Why the application path needs two policies

A connection is checked twice: the dataplane evaluates the sender's egress rules and the receiver's ingress rules separately, and both must permit it.

- Once `default-deny` selects every Pod for egress, allowing ingress on the NGINX Pods is not enough, because the client is stopped before its packet leaves.
- Enforcement comes from GKE Dataplane V2, which is already enabled, so no cluster change was needed.
...
`resolv.conf` names the DNS server and the suffixes to try, not what a Pod is allowed to resolve. `ndots:5` is why the short name `nginx` is tried as `nginx.demo.svc.cluster.local` first.
...
Applying these policies removed cluster DNS from the whole namespace, and the first version of `allow-dns` did not restore it. The diagnosis is recorded in the [troubleshooting log](../troubleshooting.md#dns-stops-resolving-after-the-default-deny).
...
Result: identical failure with no name lookup anywhere in the request. The denial is the policy. Both attempts took over two minutes because no timeout was set: a denial drops the packet silently, so `curl` waits out its own connect timeout rather than receiving a refusal. `-m 5` caps that.
...
Result: both paths returned `200 OK` five seconds apart, on a Pod that was never restarted and stayed at zero restarts. Policy is recomputed from current labels, so a label change takes effect on a running Pod within about a second.
...
Result: the namespace returned to two Pods, 100m CPU and 128Mi requested. Both NGINX Pods are two days old with zero restarts, which is the evidence that the kubelet health probes kept reaching them through the default deny throughout. Probe traffic originates on the node rather than from a Pod, and Dataplane V2 permits it without a rule.

### Deliberate omissions

- **No egress rule on the NGINX Pods.** Serving static content requires no outbound connection, so the workload cannot open one. Revisit in Phase 5, when the image is replaced, and Phase 7, when load balancer traffic arrives from outside the namespace.
- **Clients are selected by Pod label, not namespace.** That keeps the rule readable while the only client lives in `demo`, and needs a `namespaceSelector` once something outside the namespace has to reach the Service.

## Known gaps

- The workload still runs as root, so it cannot satisfy `restricted`. Phase 5 replaces it with a non-root image, after which `enforce` can be raised.
- `audit` findings are written to the Google Cloud audit log and have not been reviewed.
- Network policy denials are not logged. Every diagnosis here was made by testing from inside a Pod, which works but does not scale. Dataplane V2 policy logging is the tool for this and is not enabled.
